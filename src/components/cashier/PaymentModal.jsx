...
const METHODS = [
  { id: 'cash',        Icon: Banknote,   label: 'Cash',        ref: false },
  { id: 'easypaisa',   Icon: Phone,      label: 'EasyPaisa',   ref: true  },
  { id: 'jazzcash',    Icon: Smartphone, label: 'JazzCash',    ref: true  },
  { id: 'bank',        Icon: Building2,  label: 'Bank',        ref: true  },
  { id: 'card',        Icon: CreditCard, label: 'Card',        ref: true  },
];
const REF_LABEL = { easypaisa:'EasyPaisa Ref #', jazzcash:'JazzCash Ref #', bank:'Account / Ref #', card:'Last 4 Digits' };
...
const PaymentModal = ({ isOpen, bill, onClose, onConfirm, isOffline = false }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      setMethod('cash'); setAmount(''); setRef('');
  ...
  const validate = () => {
    if (method === 'cash' && (!amount || recv < total)) {
      toast.error(`Need at least Rs.${fmt(total)}`); triggerShake(); return false;
    }
    const m = METHODS.find(x=>x.id===method);
    if (m?.ref && !ref.trim()) {
      toast.error(`Enter ${REF_LABEL[method]||'reference'}`); triggerShake(); return false;
    }
    if (discAmt > 0 && !discReason.trim()) {
      toast.error('Discount reason required'); triggerShake(); return false;
    }
    return true;
  };
  ...
  const handleConfirm = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Payment failed: '+e.message); playError();
        ...
        >
          ...
          <div className={`flex items-center justify-between px-5 py-4 border-b ${div}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className={`font-bold text-base ${tx}`}>Receive Payment</h2>
          ...
          <div className="p-5 space-y-4 max-h-[68vh] overflow-y-auto">
            ...
            <div className={`rounded-xl px-4 py-3 ${rBg}`}>
              <div className="flex justify-between items-center">
                <span className={`text-xs ${mu}`}>Customer</span>
                <span className={`text-sm font-medium ${tx}`}>{bill.customerName||bill.customer?.name||'Walk-in'}</span>
              </div>
              <div className="flex justify-between items-center mt-1">
                <span className={`font-semibold text-sm ${tx}`}>Amount Due</span>
            ...
            {/* Method selector */}
            <div>
              <p className={`text-xs font-semibold uppercase tracking-wider mb-2 ${mu}`}>Payment Method</p>
            ...
            {/* Discount */}
            <div>
              <p className={`text-xs font-semibold mb-1.5 ${mu}`}>Discount <span className={`font-normal ${mu}`}>(optional)</span></p>
              <div className="flex gap-2">
                ...
                <input type="number" value={discVal} onChange={e=>setDiscVal(e.target.value)}
                  placeholder="0" min="0" max={discPct?100:base}
            ...
            {method === 'cash' && (
              <div>
                <p className={`text-xs font-semibold mb-1.5 ${mu}`}>Amount Received</p>
            ...
            <div className={`rounded-xl p-4 space-y-2 ${rBg}`}>
              {discAmt>0 && <>
                <div className={`flex justify-between text-sm ${mu}`}><span>Subtotal</span><span className={tx}>Rs.{fmt(base)}</span></div>
                <div className="flex justify-between text-sm"><span className={mu}>Discount</span><span className="text-green-400">-Rs.{fmt(discAmt)}</span></div>
              </>}
              ...
              {method==='cash' && recv>0 && (
                <div className={`flex justify-between font-bold border-t ${div} pt-2`}>
                  <span className={tx}>Change</span>
                  <span className={change>=0?'text-emerald-400':'text-red-400'}>
                    Rs.{fmt(Math.abs(change))}{change<0?' (short)':''}
                  </span>
                </div>
              )}
            </div>
            ...
            {!isOnline && (
              <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl bg-blue-500/10 border border-blue-500/20">
                ...
                <p className="text-xs text-blue-400">Offline — payment saved locally, syncs on reconnect.</p>
              </div>
            )}
          </div>
