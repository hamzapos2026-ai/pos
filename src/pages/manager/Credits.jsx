...
const Credits = () => {
  ...
  const loader = useCallback(
    () => managerService.getBills({ paymentStatus: 'unpaid', limit: 10000 })
      .then(r1 => managerService.getBills({ paymentStatus: 'partial', limit: 10000 })
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        <div>
          <p className="text-sm text-gray-200">{truncate(row.customer?.name || row.customerName || 'Walk-in', 22)}</p>
          <p className="text-[10px] text-gray-500">{row.customer?.phone || row.customerPhone || ''}</p>
        </div>
      ),
    },
    {
      label: 'Total', field: 'totalAmount', align: 'right',
      render: (row) => <span className="text-sm">{formatPKR(row.totalAmount)}</span>
    },
    {
      label: 'Paid', field: 'paidAmount', align: 'right',
      ...
      <div className="p-3 rounded-xl border border-[#2a1f0d] bg-[#1a1208]">
        <div className="flex justify-between items-start mb-2">
          <div>
            ...
            <p className="text-xs text-gray-400">{row.customer?.name || row.customerName || 'Walk-in'}</p>
            <p className="text-[10px] text-gray-500">{getRelativeTime(row.savedAt)}</p>
          </div>
        ...
        >
          <DollarSign className="w-3 h-3 inline mr-1" /> Collect Payment
        </button>
      </div>
    );
  };
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
        <h2 className="text-xl font-bold text-gray-100 flex items-center gap-2">
          <CreditCard className="w-5 h-5 text-amber-500" /> Credit / Pending Payments
        </h2>
        <ExportMenu data={exportRows} filename={'credits_' + new Date().toISOString().slice(0, 10)}
          pdfOptions={{ title: 'Credits Report', headers: Object.keys(exportRows[0] || {}), rows: exportRows.map(r => Object.values(r)) }} />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <StatCard label="Total Outstanding" value={stats.totalOutstanding} prefix="Rs " icon={AlertCircle} color="red" loading={loading} />
        <StatCard label="Pending Bills" value={stats.totalBills} icon={CreditCard} color="amber" loading={loading} />
        <StatCard label="Overdue (30+ days)" value={stats.overdueBills} icon={AlertCircle} color="orange" loading={loading} />
      </div>
      ...
      <DataTable
        ...
        data={filtered}
        loading={loading}
        emptyMessage="No pending credits"
        emptySubtext="All bills are fully paid 🎉"
        rowKey="localId"
